...
try:
    print(data_dir)
    graph_file = data_dir.split('/')[-2][:-9] + '.edge'
    graph_dir = '/'.join(data_dir.split('/')[:-2])
    try:
        g = nx.read_edgelist(graph_dir+'/'+graph_file,nodetype=int)
    except:
        g = nx.read_edgelist(graph_dir+'/'+graph_file,edgetype=str,delimiter=',')

except:
    try:
        graph_file = data_dir.split('/')[-2][:-3] + '.txt'
        graph_dir = '/'.join(data_dir.split('/')[:-3])
        try:
            g = nx.read_edgelist(graph_dir+'/'+graph_file,nodetype=int)
        except:
            g = nx.read_edgelist(graph_dir+'/'+graph_file,edgetype=str,delimiter=',')
    except:
        graph_file = data_dir.split('/')[-2][:-8] + '.txt'
        graph_dir = '/'.join(data_dir.split('/')[:-8])
        try:
            g = nx.read_edgelist(graph_dir+'/'+graph_file,nodetype=int)
        except:
            g = nx.read_edgelist(graph_dir+'/'+graph_file,edgetype=str,delimiter=',')
