...
# Extract O
if len(X.shape) == 1:
    O = np.array([int(2*g.number_of_edges()/g.number_of_nodes())]).reshape(1,1)
else:
    O = nd.extract_O(g,X)
...
nx.write_edgelist(test.g,f'{recon_file}/graph_detach_{detachment_rate}.txt')
np.savetxt(f'{recon_file}/comp_size_detach_{detachment_rate}.txt',test.lccs)
np.savetxt(f'{recon_file}/num_comp_detach_{detachment_rate}.txt',test.ccs)
np.savetxt(f'{recon_file}/ts_detach_{detachment_rate}.txt',test.ts)
