...
for j, net in enumerate(networks_to_consider):
    ...
    # add line to file
    if mcmc:
        line = f'--graph_file {robot_path}/{subdir}/edgefiles/{graph_file} --X_file {X_file} --num_samples 100000 --output {robot_path}/{subdir}/treefiles/'
    else:
        line = f'--graph_file {robot_path}/{subdir}/edgefiles/{graph_file} --X_file {X_file} --tree_file {robot_path}/{subdir}/treefiles/{base_name}_tree.json --output {robot_path}/stats/assembly_stats.csv'
